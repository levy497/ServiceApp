...
class HistoriaZdarzen(db.Model):
    __tablename__ = 'historia_zdarzen'

    id = db.Column(db.Integer, primary_key=True)
    timestamp = db.Column(db.DateTime)
    usterki_id = db.Column(db.Integer, db.ForeignKey('usterki.id'))
    typ_zdarzenia_id = db.Column(db.Integer, db.ForeignKey('typ_zdarzenia.id'))
    tresc_zdarzenia = db.Column(db.Text)

class TypZdarzenia(db.Model):
    __tablename__ = 'typ_zdarzenia'

    id = db.Column(db.Integer, primary_key=True)
    nazwa = db.Column(db.String(255), nullable=False)

    historia_zdarzen = db.relationship('HistoriaZdarzen', backref='typ_zdarzenia', lazy=True)
